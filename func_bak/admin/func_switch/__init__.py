...
from core_bak.model import AGroupModel, FuncType, FuncItem
...
async def main(
    ...
    # 获取所有功能列表
    func_list = [
        (func_id, FuncItem(**channel.meta))
        for func_id, channel in saya.channels.items()
    ...
    if arg_all.result:
        ...
        # 获取指定的功能id列表
        func_id_list = arg_func_ids.display.split()
        for func_id in func_id_list:
            ...
            if not func or not meta:
                return await app.send_group_message(group, "输入的功能不存在，无法开启/关闭")

            # 如果指定的功能正在维护，则返回错误信息
            if meta.maintain and not meta.can_be_disabled:
                return await app.send_group_message(group, "输入的功能正在维护或为必要功能，无法开启/关闭")
            else:
    ...
    function = action_to_function[action.display]
    ...
    await app.send_group_message(
        group,
        f"功能{action.display}完成，共{len(need_change)}个功能，成功{action_success}个，失败{action_fail}个",
    )
