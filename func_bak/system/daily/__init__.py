...
from core_bak.db_model import AUser
from core_bak.model import FuncType
from core_bak.model import AUserModel
...
async def main(app: Ariadne):
    ...
    await app.send_friend_message(
        ...
        f"{'{:.2%}'.format(sign_num/all_num) if all_num > 0 else 'NaN'}\n"
        f"活跃率 {str(chat_num)} / {str(all_num)} "
        f"{'{:.2%}'.format(chat_num/all_num) if all_num > 0 else 'NaN'}\n"
        f"活跃签到率 {str(sign_num)} / {str(chat_num)} "
        f"{'{:.2%}'.format(sign_num/chat_num) if chat_num > 0 else 'NaN'}\n"
...
async def ladder_rent_collection():
    ...
    async for user in user_list:
        ...
        await auser.reduce_coin(reduce_coin, source="梯度持有税")
        logger.info(f"[Task.daily] {user.qid} 被收取了 {reduce_coin} 游戏币")
    return total_rent
