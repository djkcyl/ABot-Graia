...
async def pornhub_style_logo_generator(app: GraiaMiraiApplication, message: MessageChain, group: Group):
    ...
    try:
        ...
        try:
           ...
           # try:
        #        await app.revokeMessage(message.getFirst(Source).id)
       #     except PermissionError:
         #       app.logger.error("机器人不为管理，无法撤回！")
        except AccountMuted:
            pass
    except ValueError:
        try:
            await app.sendGroupMessage(group, MessageChain.create([Plain(text="使用格式：ph text1 text2")]))
        except AccountMuted:
            pass
...
async def create_left_part_img(text: str, font_size: int):
    font = ImageFont.truetype(
        './saya/PornhubStyleLogoGenerator/ttf/ArialEnUnicodeBold.ttf', font_size)
    font_width, font_height = font.getsize(text)
    offset_y = font.font.getsize(text)[1][1]
    blank_height = font_height * LEFT_PART_VERTICAL_BLANK_MULTIPLY_FONT_HEIGHT
    right_blank = int(font_width / len(text) *
                      LEFT_PART_HORIZONTAL_BLANK_MULTIPLY_FONT_WIDTH)
...
async def create_right_part_img(text: str, font_size: int):
    ...
    left_blank = int(font_width / len(text) *
    ...
    circle = IMG.new(
        'L', (magnified_radii * 2, magnified_radii * 2), 0)  # 创建一个黑色背景的画布
...
async def combine_img(left_text: str, right_text, font_size: int, out_put_path: str):
    ...
    bg_img.paste(right_img, (blank + left_img.width,
                 int((bg_img_height - right_img.height) / 2)), mask=right_img)
...
async def make_ph_style_logo(left_text: str, right_text: str) -> MessageChain:
    ...
    out_put_path = f"./saya/PornhubStyleLogoGenerator/temp/{img_name}"
    if not os.path.exists("./saya/PornhubStyleLogoGenerator/temp"):
        os.mkdir("./saya/PornhubStyleLogoGenerator/temp")
