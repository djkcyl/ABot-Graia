...
AUTH = (yaml_data['Saya']['RecordQuery']['r6']['user_id'], yaml_data['Saya']['RecordQuery']['r6']['password'])
    ...
    if PICPATH.exists():
        ...
        async with httpx.AsyncClient(timeout=10) as client:
            for _ in range(3):
                try:
                    if type == "operators":
                        r = await client.get(f"https://api.statsdb.net/r6/assets/operators/{name}/figure/small", timeout=10)
                    elif type == "weapons":
                        r = await client.get(f"https://api.statsdb.net/r6/assets/weapons/{name}", timeout=10)
                    break
...
async def draw_r6(nick_name):
    ...
    for _ in range(3):
        try:
            avatar = Image.open(BytesIO(httpx.get(data["payload"]["user"]["avatar"]).content))
            break
        except httpx.HTTPError:
            continue
    ...
    draw.text((222, 115), f"Level {level}   Playtime {time}   MMR {mmr}", (200, 200, 200), font_regular_28)
    ...
    kpm = '%.2f' % division_zero(data['payload']['stats']['general']['kills'], data['payload']['stats']['general']['matchesplayed'])
    draw.text((180 * 1 - 96, 340), kpm, "white", font_bold_32)
    draw.text((180 * 2 - 94, 340), str(data['payload']['stats']['general']['kills']), "white", font_bold_32)
    draw.text((180 * 3 - 95, 340), str(data['payload']['stats']['general']['deaths']), "white", font_bold_32)
    draw.text((180 * 4 - 94, 340), str(data['payload']['stats']['general']['assists']), "white", font_bold_32)

    wl = '%.2f%%' % (data['payload']['stats']['general']['wins'] / data['payload']['stats']['general']['matchesplayed'] * 100)
    draw.text((180 * 1 - 96, 432), wl, "white", font_bold_32)
    draw.text((180 * 2 - 94, 432), str(data['payload']['stats']['general']['wins']), "white", font_bold_32)
    draw.text((180 * 3 - 95, 432), str(data['payload']['stats']['general']['losses']), "white", font_bold_32)
    draw.text((180 * 4 - 94, 432), str(data['payload']['stats']['general']['matchesplayed']), "white", font_bold_32)

    kd = '%.2f' % division_zero(data['payload']['stats']['general']['kills'], data['payload']['stats']['general']['deaths'])
    draw.text((180 * 1 - 96, 520), str(data['payload']['stats']['general']['headshots']), "white", font_bold_32)
    draw.text((180 * 2 - 94, 520), str(data['payload']['stats']['general']['dbno']), "white", font_bold_32)
    draw.text((180 * 3 - 95, 520), str(data['payload']['stats']['general']['gadgetdestroy']), "white", font_bold_32)
    draw.text((180 * 4 - 94, 520), kd, "white", font_bold_32)

    # 常用干员
    most_played = sorted(data['payload']['stats']['operators'].copy(), key=lambda l1: l1["timeplayed"], reverse=True)[0]
    best_kd = sorted(data['payload']['stats']['operators'].copy(), key=lambda l2: (division_zero(l2["kills"], l2["deaths"]), l2["timeplayed"]), reverse=True)[0]
    best_wl = sorted(data['payload']['stats']['operators'].copy(), key=lambda l3: (division_zero(l3["wins"], l3["losses"]), l3["timeplayed"]), reverse=True)[0]
    ...
    weapons = sorted(data['payload']['stats']['weaponDetails'].copy(), key=lambda lilt: lilt["kills"], reverse=True)[0]
