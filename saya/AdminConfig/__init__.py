...
async def group_func(
    ...
    if reg1.matched:
        ...
        if ctrl == "修改":
            if not reg2.matched:
                return await safeSendGroupMessage(
                    group, MessageChain.create("请输入需要修改的配置名称")
                )
            ...
            for config in configList:
                if config["name"] == configname:
                    if not config["can_edit"]:
                        return await safeSendGroupMessage(
                            group,
                            MessageChain.create(f"{configname} 不可修改"),
                        )
                    if not reg3.matched:
                        return await safeSendGroupMessage(
                            group, MessageChain.create("请输入修改后的值")
                        )
                    ...
                    return await safeSendGroupMessage(
                        group,
                        MessageChain.create(f"{configname} 已修改为 {config_Message}"),
                    )
            return await safeSendGroupMessage(
                group, MessageChain.create(f"{configname} 不存在")
            )
        elif ctrl == "查看":
            if not reg2.matched:
                return await safeSendGroupMessage(
                    group, MessageChain.create("请输入需要查看的配置名称")
                )
            configname = reg2.result.getFirst(Plain).text
            for config in configList:
                if config["name"] == configname:
                    return (
                        await safeSendGroupMessage(
                            group,
                            MessageChain.create(f"{configname} 当前值为 {config_Message}"),
                        )
                        if (
                            ...
                            group,
                            MessageChain.create(f"{configname} 当前值为 空"),
                        )
                    )
            ...
            if not reg2.matched:
                return await safeSendGroupMessage(
                    group, MessageChain.create("请输入需要关闭的配置名称")
                )
            configname = reg2.result.getFirst(Plain).text
            for config in configList:
                if config["name"] == configname:
                    ...
                    if not group_data[str(group.id)][configkey]["Enabled"]:
                        return await safeSendGroupMessage(
                            group,
                            MessageChain.create(f"{configname} 已处于关闭状态"),
                        )
                    group_data[str(group.id)][configkey]["Enabled"] = False
                    save_config()
                    return await safeSendGroupMessage(
                        group, MessageChain.create(f"{configname} 已关闭")
                    )
            return await safeSendGroupMessage(
                group, MessageChain.create(f"{configname} 不存在")
            )
            ...
            if not reg2.matched:
                return await safeSendGroupMessage(
                    group, MessageChain.create("请输入需要开启的配置名称")
                )
            configname = reg2.result.getFirst(Plain).text
            for config in configList:
                if config["name"] == configname:
                    ...
                    if group_data[str(group.id)][configkey]["Enabled"]:
                        return await safeSendGroupMessage(
                            group,
                            MessageChain.create(f"{configname} 已处于开启状态"),
                        )
                    group_data[str(group.id)][configkey]["Enabled"] = True
                    save_config()
                    return await safeSendGroupMessage(
                        group, MessageChain.create(f"{configname} 已开启")
                    )
            return await safeSendGroupMessage(
                group, MessageChain.create(f"{configname} 不存在")
            )
            ...
            return await safeSendGroupMessage(
                group, MessageChain.create("请输入修改|查看|关闭|开启")
            )
    else:
        msg = "当前可调整的群功能有："
