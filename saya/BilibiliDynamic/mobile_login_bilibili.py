...
login_cache_file = Path(__file__).parent.joinpath("login_cache.json")
...
class bilibiliMobile:
    ...
    async def login(self):
        if login_cache_file.exists():
            response_json = json.loads(login_cache_file.read_text())
        else:
            app = get_running(Ariadne)
            # 加密
            ...
            login_cache_file.write_text(json.dumps(response_json))
        # 登录成功
    ...
    async def loginbysms(self) -> dict:
        ...
        if response.json()["code"] == 0:
            ...
            await app.sendFriendMessage(
                yaml_data["Basic"]["Permission"]["Master"],
                MessageChain.create("[BiliBili推送] 验证码已发送至您的手机，请在 5 分钟内完成验证"),
            )
            ...
            await app.sendFriendMessage(
                yaml_data["Basic"]["Permission"]["Master"],
                MessageChain.create(
                    f"[BiliBili推送] {self.username} 验证码发送失败，ABot 已关闭\n{response.json()}"
                ),
            )
            exit()
    ...
    def __initialize(self):
        ...
        self.headers = {
            ...
            "User-Agent": "Mozilla/5.0 BiliDroid/6.70.0 (bbcallen@gmail.com) os/android model/KB2000 mobi_app/android build/6700300 channel/yingyongbao innerVer/6700310 osVer/12 network/2",
        }
