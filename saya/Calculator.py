...
async def calculator_main(app: Ariadne, group: Group, message: MessageChain, source: Source):
    ...
    if len(saying) == 2:
        ...
        if len(expression) > 800:
            return await safeSendGroupMessage(group, MessageChain.create([Plain("字符数过多")]), quote=source.id)
        try:
            answer = await asyncio.wait_for(asyncio.to_thread(arithmetic, expression), timeout=15)
        except ZeroDivisionError:
            return await safeSendGroupMessage(group, MessageChain.create([Plain("0不可作为除数")]), quote=source.id)
        except asyncio.TimeoutError:
            return await safeSendGroupMessage(group, MessageChain.create([Plain("计算超时")]), quote=source.id)
        except Exception:
            return await safeSendGroupMessage(group, MessageChain.create([Plain("出现未知错误，终止计算")]), quote=source.id)

        await safeSendGroupMessage(group, MessageChain.create([Plain(answer)]), quote=source.id)
