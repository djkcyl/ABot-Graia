...
async def anime_search(app: Ariadne, group: Group, member: Member, message: MessageChain, source: Source):
    ...
    if V_RUNING:
        await safeSendGroupMessage(group, MessageChain.create([Plain("以图搜番正在运行，请稍后再试")]))
    else:
        if message.has(Image):
            image_url = message.getFirst(Image).url
        else:
            WAITING.append(member.id)
            waite = safeSendGroupMessage(group, MessageChain.create([Plain("请发送图片以继续，发送取消可终止搜番")]))
            try:
        ...
        if await reduce_gold(str(member.id), 4):
            ...
            await safeSendGroupMessage(group, MessageChain.create([Image(data_bytes=image)]), quote=source.id)
...
async def saucenao(app: Ariadne, group: Group, member: Member, message: MessageChain, source: Source):
    ...
    if I_RUNING:
        await safeSendGroupMessage(group, MessageChain.create([Plain("以图搜图正在运行，请稍后再试")]))
    else:
        if message.has(Image):
            image_url = message.getFirst(Image).url
        else:
            WAITING.append(member.id)
            waite = await safeSendGroupMessage(group, MessageChain.create([Plain("请发送图片以继续，发送取消可终止搜图")]))
            try:
                image_url = await asyncio.wait_for(inc.wait(waiter1), timeout=20)
                if not image_url:
                    WAITING.remove(member.id)
        ...
        if await reduce_gold(str(member.id), 4):
            I_RUNING = True
            await safeSendGroupMessage(group, MessageChain.create([Plain("正在搜索，请稍后")]), quote=source.id)
