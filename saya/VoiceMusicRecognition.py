...
config = {
    'host': yaml_data['Saya']['VoiceMusicRecognition']['host'],
    'access_key': yaml_data['Saya']['VoiceMusicRecognition']['access_key'],
    'access_secret': yaml_data['Saya']['VoiceMusicRecognition']['access_secret'],
    'timeout': 15
}
are = ACRCloudRecognizer(config)
...
async def repeater(app: GraiaMiraiApplication, group: Group, member: Member, source: Source):
    ...
    if member.id not in WAITING:
        ...
        await app.sendGroupMessage(group, MessageChain.create([Plain(f"请通过语音发送想要识别的歌曲，5至10秒即可，发送取消可终止识别，无论识别成功与否均需扣除 2 游戏币")]))
        try:
            voice_url = await asyncio.wait_for(inc.wait(waiter), timeout=60)
            if not voice_url:
                WAITING.remove(member.id)
                return await app.sendGroupMessage(group, MessageChain.create([Plain("已取消识别")]))
        ...
        if await reduce_gold((member.id), 2):
            ...
            voice_mp3 = await silkcoder.decode(voice, audio_format="mp3")
            voice_info = are.recognize_by_filebuffer(voice_mp3, 0, 15)
            voice_info = json.loads(voice_info)
            music_list = []
            if voice_info['status']['code'] == 0:
                i = 1
                for music in voice_info['metadata']['music']:
                    ...
                    music_list.append(f"{i}  ====>  | {music_name} <==> {music_artists} <==> {music_album} | 相似度：{music_score}")
                    i += 1
                image = await create_image(
                    str("为你找到以下可能的歌曲：   曲名 <==> 艺术家 <==> 专辑\n" +
                        "=====================================================\n" +
                        "\n".join(music_list)), 120)
                ...
                await app.sendGroupMessage(group, MessageChain.create([Plain(f"{voice_info['status']['code']} 未从语音中识别到歌曲，请检查语音音量是否过小或音质过于嘈杂")]))
        else:
            await app.sendGroupMessage(group, MessageChain.create([Plain("你的游戏币不足，无法使用")]))
        WAITING.remove(member.id)
